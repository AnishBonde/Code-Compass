...
def preprocess_text(text): #converts url into proper sentence
    text = re.sub(r"http\S+|www\S+|https\S+", '', text, flags=re.MULTILINE)
    text = re.sub(r'[^A-Za-z]+', ' ', text)
    text = text.lower()
    words = word_tokenize(text)   #Tokenization: Splits the text into individual words.
    stemmer = PorterStemmer()
    stemmed_words = [stemmer.stem(word) for word in words if word not in stopwords.words('english')]
    text = ' '.join(stemmed_words)
    return text

def preprocess_and_tokenize(text, tokenizer): #preproprocesses text by converting words by lemmentizing and then converting these lemetized into bert tokens basically 0s and 1s
    text = text.lower() 
...
def calculate_similarity():
    ...
    count = 0
    # ur=unique repo
# unique intressted commented
# up-unique pull request 
#all this is fetched from react 
    ...
    for known_issue in toComparedIssues:
        for unknown_issue in suggestedRepo:
            ...
            tokenizer = BertTokenizer.from_pretrained("bert-base-uncased") #word to vec
